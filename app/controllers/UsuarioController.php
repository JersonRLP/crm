...
class UsuarioController
{
    ...
    public function newUser()
    {
        ...
        if ($resultado) {
            // Suponiendo que el usuario logueado está en $_SESSION['idusuario']
            $idUsuarioActual = $_SESSION['idusuario'] ?? null;

            if ($idUsuarioActual) {
                $this->LogModel->registrarLog(
                    $idUsuarioActual,
                    'Usuario Registrado',
                    "Se registró al nuevo usuario '$nombres' con ID $resultado",
                    $resultado,
                    'usuario'
                );
            }
    ...
    public function EditUser()
    {
        ...
        if ($idUsuarioActual) {
            $this->LogModel->registrarLog(
                $idUsuarioActual,
                'Usuario Modificado',
                "Se modificó al usuario '$nombres' con ID $idusuario",
                $idusuario,
                'usuario'
            );
        }
